...
def calcula_porcentagem_sorteio(assinante,minutos_assistidos):
    ...
    index = 0
    zipp = tuple(zip(assinante, minutos_assistidos))
    tempos = []
    percentual = []
    
    #roda os loops até que o ultimo usuário seja verificado.
    while index < len(assinante):
        #verifica se o usuário é assinante ou não.
        if zipp[index][0] == True:
            #verifica o tempo assistido, caso seja 0, 0 será anexado ao tempo
            if zipp[index][1] == 0:
                tempos.append(0)
                index += 1
            #caso haja algum registro de tempo o tempo é arredondado para cima
            #e logo em seguida duplicado e anexado ao tempo.
            else:    
                r = zipp[index][1] / 60
                #porém, caso o tempo esteja entre 0 e 1. 1 é usado para o calculo.
                if ceil(r) == 0:
                    r = 1
                tempos.append(ceil(r) * 2)
                index += 1
        
        elif zipp[index][0] == False:
            #verifica o tempo assistido, caso seja 0, 0 será anexado ao tempo
            if zipp[index][1] == 0:
                tempos.append(0)
                index += 1
            ...
            #e anexado ao tempo
                ...
                if ceil(r) == 0:
                    r = 1
                tempos.append(ceil(r))
                index += 1
    
    #para cada registro de tempo na lista de tempos, é feito um calculo percentual para
    #idenficar a probabilidade de cada usuário ser sorteado.
    for tempo in tempos:
        r = tempo/sum(tempos)
        percentual.append(round(r*100))
